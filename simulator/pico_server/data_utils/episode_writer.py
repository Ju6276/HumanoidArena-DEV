...
class EpisodeWriter():
    def __init__(self, task_dir, frequency=30,
        ...
        state_shape: [29]
        action_shape: [29]
        data_keys: list of image keys to save (e.g., ['rgb'], ['front_rgb', 'world_rgb'])
        """
        ...
        self.image_dirs = {}  # Store directory paths for each image key
    ...
    def create_episode(self):
        ...
        # Create directories for each image key
        self.image_dirs = {}
        for key in self.data_keys:
            image_dir = os.path.join(self.episode_dir, key)
            os.makedirs(image_dir, exist_ok=True)
            self.image_dirs[key] = image_dir
            print(f"==> {key}_dir: {image_dir}")

        # Keep backward compatibility with old rgb_dir attribute
        if "rgb" in self.data_keys:
            self.rgb_dir = self.image_dirs["rgb"]
    ...
    def _process_item_data(self, item_data):
        ...
        # Save all images and depth maps based on data_keys
        for key in self.data_keys:
            data = item_data.get(key, None)
            if data is not None:
                # Check if this is depth data (float32) or RGB image (uint8)
                if 'depth' in key:
                    # Save depth as .npy file (numpy format for float32 data)
                    depth_name = f'{str(idx).zfill(6)}.npy'
                    save_path = os.path.join(self.image_dirs[key], depth_name)
                    try:
                        np.save(save_path, data)
                        # Store relative path in item_data
                        item_data[key] = str(Path(save_path).relative_to(Path(self.json_path).parent))
                    except Exception as e:
                        print(f"Failed to save {key} depth data: {e}")
                else:
                    # Save RGB image as .jpg file
                    color_name = f'{str(idx).zfill(6)}.jpg'
                    save_path = os.path.join(self.image_dirs[key], color_name)
                    if not cv2.imwrite(save_path, data):
        ...
        state_neck = item_data.get('state_neck', None)
        ...
        action_neck = item_data.get('action_neck', None)

        # human data
        human_data = item_data.get('human_data', None)
        smplx_data = item_data.get('smplx_data', None)
        ...
        if state_neck is not None:
            item_data['state_neck'] = state_neck
        ...
        if action_neck is not None:
            item_data['action_neck'] = action_neck

        if human_data is not None:
            item_data['human_data'] = human_data
        if smplx_data is not None:
            item_data['smplx_data'] = smplx_data

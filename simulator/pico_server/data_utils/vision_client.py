        ...
        try:
            while self.running:
                try:
                    ...
                    depth_image = None
                    if depth_length > 0:
                        depth_start = jpeg_end + 4
                        depth_end = depth_start + depth_length

                        if len(message) >= depth_end:
                            try:
                                depth_data = message[depth_start:depth_end]
                                # Decode depth data as float32 array
                                depth_image = np.frombuffer(depth_data, dtype=np.float32)
                                depth_image = depth_image.reshape(height, width)
                            except Exception as e:
                                if verbose:
                                    print(f"[Warning] Depth decode error: {e}")
                                depth_image = None

                    have_color_data = True
                    have_depth_data = depth_image is not None
                    ...
                    if have_depth_data:
                        self.handle_depth_image(depth_image)

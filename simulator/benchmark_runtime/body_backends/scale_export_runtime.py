"""Official ScaleBFM export math/wrapper, extracted without Isaac app startup.
Source: ScaleTrack/scripts/pretrain/rsl_rl/play_export_check_humanoid_transformer.py
See external/ScaleBFM/LICENSE for upstream license.
"""
...
class HumanoidTransformerPolicyWrapperWithMode(nn.Module):
    ...
    def forward(self, root_quat_buffer: torch.Tensor, base_ang_vel_buffer: torch.Tensor, dof_pos_buffer: torch.Tensor, dof_vel_buffer: torch.Tensor, last_action_buffer: torch.Tensor, target_body_pos_future_to_robot_base: torch.Tensor, target_body_rot_future_to_robot_base: torch.Tensor, mode_index: torch.Tensor, time_offsets: torch.Tensor) -> torch.Tensor:
        ...
        prop_obs = torch.cat([projected_gravity_buffer, base_ang_vel_buffer, dof_pos_rel_buffer, dof_vel_buffer * 0.05], dim=-1)
        ...
        target_body_rot_future_to_robot_base_tan_norm = torch.cat([quat_apply(target_body_rot_future_to_robot_base, self.tan_vec), quat_apply(target_body_rot_future_to_robot_base, self.norm_vec)], dim=-1)
        target_body_rot_future_rel_to_robot_base = quat_mul_inverse_right(target_body_rot_future_to_robot_base, body_quat_to_robot_base[:, None].expand(-1, target_body_rot_future_to_robot_base.shape[1], -1, -1))
        target_body_rot_future_rel_to_robot_base_tan_norm = torch.cat([quat_apply(target_body_rot_future_rel_to_robot_base, self.tan_vec), quat_apply(target_body_rot_future_rel_to_robot_base, self.norm_vec)], dim=-1)
        task_obs = torch.cat([target_body_pos_future_to_robot_base.flatten(2, 3), target_body_pos_future_rel_to_robot_base.flatten(2, 3), target_body_rot_future_to_robot_base_tan_norm.flatten(2, 3), target_body_rot_future_rel_to_robot_base_tan_norm.flatten(2, 3), time_offsets], dim=-1)
...
def build_mode_mappings(mode_table: torch.Tensor, feature_dims_per_link, with_time: bool=True, device: torch.device | None=None) -> torch.Tensor:
    ...
    for dim in feature_dims_per_link:
        part = (torch.ones(num_modes, mode_table.shape[1], dim, device=device, dtype=torch.float32) * mode_table.unsqueeze(-1)).view(num_modes, -1)
...
def parse_xml(xml_file, device='cpu'):
    ...
    joint_axis = torch.stack(joint_axis, dim=0).float().to(device)

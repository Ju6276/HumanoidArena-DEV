...
class G129dofRobotBaseCfg:
    ...
    def get_base_config(
        ...
        if update_default_joint_pos:
            ...
            if use_twist2_defaults:
                # TWIST2 MuJoCo default_dof_pos (29-DoF body). See `server_low_level_g1_sim.py`.
                # Legs
                ...
                # Hands: TWIST2 29-DoF body does not include hand joints; keep zeros if enabled.
                if is_have_hand:
                    default_joint_pos.update(G1RobotJointTemplates.get_hand_joints(hand_type))

            else:
                # Legacy: all zeros from templates
                # add the leg joints
